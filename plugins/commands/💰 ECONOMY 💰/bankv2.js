  ...
  try {
    ...
    switch (args[0]) {
      ...
      case 'r': {
        ...
        if (res.status == false) return message.reply(`[ 𝕮𝖎𝖓𝖉𝖞-🤖 ] »`, res.message);
        message.send('[ 𝕮𝖎𝖓𝖉𝖞-🤖 ] » Mật Khẩu Tài Khoản Ngân Hàng 𝐂𝐢𝐧𝐝𝐲 𝐁𝐚𝐧𝐤 của bạn là: ' + res.message.password, message.senderID);
        return message.send(`[ 𝕮𝖎𝖓𝖉𝖞-🤖 ] » \n=== [ ${res.message.noti} ] ===\n👤 Chủ sở hữu: ${res.message.name}\n💳 STK: ${res.message.STK}\n💰 Số dư: ${res.message.money}$\n🔐  Password: đã được gửi đến inbox (nếu không nhận được vui lòng addfr bot và đặt lại mật khẩu)`);
      }
      case "tim":
      case "-f": {
        if (checkBank.status == false) message.reply("[ 𝕮𝖎𝖓𝖉𝖞-🤖 ] » Bạn chưa có tài khoản trên hệ thống!")
        if (args[1] != "stk" && args[1] != "id") {
          message.reply("[ 𝕮𝖎𝖓𝖉𝖞-🤖 ] » Vui lòng chọn đúng kiểu (stk/id)")
        }
        ...
        return message.reply(`[ 𝕮𝖎𝖓𝖉𝖞-🤖 ] »\n 👤 Chủ sở hữu: ${name}\n💳 STK: ${stk}\n💰 Số dư: ${soDu}$`)
      }
      case 'thongtin':
      case '-i':
      case 'check':
      case '-c': {
        if (checkBank.status == false) return message.reply('[ 𝕮𝖎𝖓𝖉𝖞-🤖 ] » Bạn chưa có tài khoản MDL Bank!');
      ...
      case '-w':
      case 'rut': {
        if (checkBank.status == false) return message.reply('[ 𝕮𝖎𝖓𝖉𝖞-🤖 ] » Bạn chưa có tài khoản 𝐂𝐢𝐧𝐝𝐲 𝐁𝐚𝐧𝐤!');
        if (!args[1]) return message.reply('[ 𝕮𝖎𝖓𝖉𝖞-🤖 ] » Vui lòng nhập lệnh với : withdraw/-w/rut <số tiền cần rút>');
        message.reply('[ 𝕮𝖎𝖓𝖉𝖞-🤖 ] » Hãy hoàn tất bước cuối cùng ở mục INBOX với BOT (Nếu không có hãy check tin nhắn chờ hoặc hãy addfr bot)!');
        await message
          .send('[ 𝕮𝖎𝖓𝖉𝖞-🤖 ] » Vui lòng reply tin nhắn này và nhập mật khẩu tài khoản 𝐂𝐢𝐧𝐝𝐲 𝐁𝐚𝐧𝐤 của bạn để rút tiền!', message.senderID)
          .then(data => {
            data.addReplyEvent({
      ...
      case '-p': {
        if (checkBank.status == false) return message.reply('[ 𝕮𝖎𝖓𝖉𝖞-🤖 ] » Bạn chưa có tài khoản 𝐂𝐢𝐧𝐝𝐲 𝐁𝐚𝐧𝐤!');
        if (!args[1] || !args[2] || !args[3]) return message.reply('[ 𝕮𝖎𝖓𝖉𝖞-🤖 ] » Vui lòng nhập đúng lệnh: chuyentien/-p  stk/id <stk/id người nhận> <số tiền cần chuyển>');
        if (args[1] == 'stk') {
          message.reply('[ 𝕮𝖎𝖓𝖉𝖞-🤖 ] » Hãy hoàn tất bước cuối cùng ở mục INBOX với BOT (Nếu không có hãy check tin nhắn chờ hoặc hãy addfr bot)!');
          await message
            .send('[ 𝕮𝖎𝖓𝖉𝖞-🤖 ] » Vui lòng reply tin nhắn này và nhập mật khẩu tài khoản 𝐂𝐢𝐧𝐝𝐲 𝐁𝐚𝐧𝐤 của bạn để chuyển tiền!', message.senderID)
            .then(data => {
              data.addReplyEvent({
                callback: handleReply,
                type: 'transferSTK',
      ...
      case 'nap': {
        if (checkBank.status == false) return message.reply('[ 𝕮𝖎𝖓𝖉𝖞-🤖 ] » Bạn chưa có tài khoản 𝐂𝐢𝐧𝐝𝐲 𝐁𝐚𝐧𝐤!');
        if (!args[1]) return message.reply('[ 𝕮𝖎𝖓𝖉𝖞-🤖 ] » Vui lòng nhập số tiền cần nạp vào!\ndeposit/-d/nap <số tiền cần nạp>');
        var i = (await Users.getMoney(message.senderID) || null);
        var money = args[1];
        if (i < parseInt(money)) return message.reply('[ 𝕮𝖎𝖓𝖉𝖞-🤖 ] » Đell có tiền cũng đòi nạp , mua đá ở chỗ nào mà phê thế=)))');
      ...
      case '-pw': {
        ...
        switch (type) {
          case 'getpass': {
            ...
            message.reply('[ 𝕮𝖎𝖓𝖉𝖞-🤖 ] » Mật khẩu của bạn được gửi đến INBOX hoặc tin nhắn chờ!');
            return message.send(`[ 𝕮𝖎𝖓𝖉𝖞-🤖 ] » Mật khẩu của bạn là: ${res.message.password}`, message.senderID);
          }
          case 'getnew':
          case 'doimatkhau':
          case 'new': {
            message.reply('[ 𝕮𝖎𝖓𝖉𝖞-🤖 ] » Hãy hoàn tất bước cuối cùng ở mục INBOX với BOT (Nếu không có hãy check tin nhắn chờ hoặc hãy addfr bot)!');
            await message
              .send('[ 𝕮𝖎𝖓𝖉𝖞-🤖 ] » Vui lòng reply tin nhắn này để nhập mật khẩu mới!', message.senderID)
              .then(data => {
                data.addReplyEvent({
                  callback: handleReply,
                  type: 'renewPass',
      ...
      case 'gopy': {
        if (checkBank.status == false) return message.reply('[ 𝕮𝖎𝖓𝖉𝖞-🤖 ] » Bạn chưa có tài khoản 𝐂𝐢𝐧𝐝𝐲 𝐁𝐚𝐧𝐤!');
        if (!args.slice(1).join(" ")) return message.reply('[ 𝕮𝖎𝖓𝖉𝖞-🤖 ] » Vui lòng nhập nội dung cần góp ý!');
        ...
        if(res.status == false) return message.reply(`[ 𝕮𝖎𝖓𝖉𝖞-🤖 ] »` + res.message);
        return message.reply(`[ 𝕮𝖎𝖓𝖉𝖞-🤖 ] »` + res.message);
      }
        
      default: {
        return message.reply({ body: "=== 「 ...𝐂𝐢𝐧𝐝𝐲 𝐁𝐚𝐧𝐤... 」 ===\n»»» 𝐌𝐨̣̂𝐭 𝐬𝐨̂́ 𝐥𝐞̣̂𝐧𝐡 𝐡𝐢𝐞̣̂𝐧 𝐜𝐨́ 𝐭𝐫𝐞̂𝐧 𝐡𝐞̣̂ 𝐭𝐡𝐨̂́𝐧𝐠: \n\n» 1.𝙗𝙖𝙣𝙠 𝙙𝙖𝙣𝙜𝙠𝙞/-𝙧/𝙧: Để đăng kí tài khoản \n» 2.𝙗𝙖𝙣𝙠 𝙩𝙝𝙤𝙣𝙜𝙩𝙞𝙣/𝙘𝙝𝙚𝙘𝙠/-𝙘: Để xem thông tin tài khoản bản thân\n» 3.𝙗𝙖𝙣𝙠 𝙩𝙞𝙢/-𝙛 [𝙨𝙩𝙠/𝙞𝙙] [𝙨𝙤̂́ 𝙨𝙩𝙠/𝙞𝙙 𝙘𝙖̂̀𝙣 𝙩𝙞̀𝙢]: Để tìm tài khoản ngân hàng của ai đó\n» 4.𝙗𝙖𝙣𝙠 𝙧𝙪𝙩/𝙬𝙞𝙩𝙝𝙙𝙧𝙖𝙬/-𝙬 <𝙨𝙤̂́ 𝙩𝙞𝙚̂̀𝙣 𝙘𝙖̂̀𝙣 𝙧𝙪́𝙩>: Để rút tiền \n» 5.𝙗𝙖𝙣𝙠 𝙘𝙝𝙪𝙮𝙚𝙣𝙩𝙞𝙚𝙣/-𝙥 <𝙨𝙩𝙠/𝙞𝙙> <𝙨𝙤̂́ 𝙨𝙩𝙠/𝙞𝙙 𝙘𝙖̂̀𝙣 𝙘𝙝𝙪𝙮𝙚̂̉𝙣> <𝙨𝙤̂́ 𝙩𝙞𝙚̂̀𝙣 𝙘𝙖̂̀𝙣 𝙘𝙝𝙪𝙮𝙚̂̉𝙣>: Để chuyển tiền \n» 6.𝙗𝙖𝙣𝙠 𝙙𝙚𝙥𝙤𝙨𝙞𝙩/-𝙙/𝙣𝙖𝙥 <𝙨𝙤̂́ 𝙩𝙞𝙚̂̀𝙣 𝙘𝙖̂̀𝙣 𝙣𝙖̣𝙥>: Nạp tiền vào tài khoản \n» 7.𝙗𝙖𝙣𝙠 -𝙥𝙬/𝙥𝙖𝙨𝙨/𝙢𝙖𝙩𝙠𝙝𝙖𝙪 <𝙜𝙚𝙩𝙥𝙖𝙨𝙨>: lấy lại pass \n» 8.𝙗𝙖𝙣𝙠 -𝙥𝙬/𝙥𝙖𝙨𝙨/𝙢𝙖𝙩𝙠𝙝𝙖𝙪 <𝙜𝙚𝙩𝙣𝙚𝙬/𝙙𝙤𝙞𝙢𝙖𝙩𝙠𝙝𝙖𝙪/𝙣𝙚𝙬> : Đổi mật khẩu tài khoản 𝐂𝐢𝐧𝐝𝐲 𝐁𝐚𝐧𝐤 của bạn\n» 9. 𝙗𝙖𝙣𝙠 𝙜𝙤𝙥𝙮/𝙨𝙪𝙜𝙜𝙜𝙚𝙨𝙩 <𝙣𝙤̣̂𝙞 𝙙𝙪𝙣𝙜> : Để góp ý giúp api tốt hơn", attachment: anhbank });
      }
  ...
  try {
    ...
    switch (eventData.type) {
      ...
      case 'transferSTK': {
        ...
        if (res.status == false) return message.send(res.message);
        message.reply(`[ 𝕮𝖎𝖓𝖉𝖞-🤖 ] » [ ${res.message.noti} ]\n ${res.message.data.message}`);
        return message.send(`[ 𝕮𝖎𝖓𝖉𝖞-🤖 ] » [ ${res.message.noti} ]\n\n${res.message.data.message}`, eventData.tid);
      }
      case 'transferID': {
      ...
      case 'renewPass': {
        ...
        return message.send(`[ 𝕮𝖎𝖓𝖉𝖞-🤖 ] » Thay đổi mật khẩu thành công!\nMật khẩu hiện tại: ${res.message.password}`)
      }
